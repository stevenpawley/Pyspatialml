...
class indices2(object):
    """Calculate remote sensing spectral indices using Raterio and Numpy"""
    def __init__(self, raster, blue=None, green=None, red=None, nir=None,
                 swir1=None, swir2=None, swir3=None):
        """Define new spectral indices class
        ...
        ----------
        raster : str
            Path to GDAL file with stacked imagery bands
        blue : int
            Index of blue band.
        green : int
            Index of green band.
    ...
    def applier(self, func, file_path, **kwargs):
        ...
        for i, window in self.src.block_windows():
            # print_progressbar(i, rows, length=50)

            img = self.src.read(masked=True, window=window)
            result = func(img, **kwargs)

            if i[0] == 0:
                func_output = rasterio.open(
                    file_path, mode='w', driver='GTiff',
                    height=self.src.height, width=self.src.width,
                    count=result.shape[0], dtype='float32',
                    crs=self.src.meta['crs'], transform=self.src.transform,
                    nodata=-99999)
            ...
            if len(result.shape) == 2:
                ...
                indexes = [i for i in range(result.shape[0])]

            func_output.write(
                result.astype('float32'), window=window, indexes=indexes)

        func_output.close()

        return func_output

    def dvi(self, s, file_path):
        """Difference vegetation index (DVI)
        Richardson, A. J., and Weigand, C. (1977) Distinguishing vegetation
        from soil background information. Photogrammetric Engineering and
        Remote Sensing, p. 43.
        ...
        def f(img, **kwargs):
            ...
            nir = kwargs['nir']
            red = kwargs['red']
            return s * img[nir, :, :] - img[red, :, :]

        output = self.applier(f, file_path, **{
                's': s, 'red': self.red, 'nir': self.nir})

        return output

    def ctvi(self, file_path):
        """Corrected transformed vegetation index (CTVI)
        Perry, C. R., Jr. and Lautenschlager, L.F. (1984) Functional
        equivalence of spectral vegetation indices.
        Remote Sens of Environ v.14, 169-182.
