...
class TestToCrs(TestCase):
    ...
    def test_to_crs_defaults(self):
        ...
        self.assertEqual(stack_prj.read(masked=True).min(), self.stack.read(masked=True).min())
        self.assertEqual(stack_prj.read(masked=True).max(), self.stack.read(masked=True).max())
